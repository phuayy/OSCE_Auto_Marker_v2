...
| `0009_sessions_created_by` | Adds `sessions.created_by` (nullable, indexed) — the creator's user id, mirrored out of the `createdBy` snapshot in the payload so "sessions by this account" is an indexed query. No backfill: earlier rows genuinely have no creator. Mirrors the entry in `ADDITIVE_MIGRATIONS`, which adds the column (not the index). |
