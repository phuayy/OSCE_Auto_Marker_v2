"""Durable token revocation, and change tracking on the table.

Revision ID: 0015
Revises: 0014
Create Date: 2026-09-18

Logout used to revoke a bearer token in an in-process ``dict``
(``core/token_revocation.py``). A restart resurrected every logged-out token
until its own expiry, and a second process (a Hatchet worker aside, which
never verifies a token) never learned of a revocation another process made.
This table makes a revocation a database fact instead, and change tracking
turns a revocation in one process into an eviction in every other one within
milliseconds — the same story ``users``' trigger already tells for a
disabled account or a token-version bump.

``token_id`` is the JWT's own ``tokenId`` claim, not a surrogate — that is
the only thing verification has to check. ``expires_at`` mirrors the token's
own expiry so a background purge (mirroring
``UserRepository.purge_spent_tokens``) can drop a row once the token it names
could never be replayed again, keeping this table's size bounded by the
token TTL rather than by how many people have ever logged out.
"""
...
# 0005, 0006, 0008, 0011 and 0014: a migration must keep meaning what it
# meant when written, even if the live trigger SQL changes later.
_PG_FUNCTION = f"""
...
def _install_change_tracking() -> None:
    ...
    if is_postgres:
        try:
            ...
            logger.warning(
                "Could not create the change-tracking trigger function; token revocation "
    ...
    try:
        ...
        logger.warning(
            "Could not install change tracking on '%s' (%s); a revocation will reach other "
...
def upgrade() -> None:
    if not _has_table(_TABLE):
        ...
        op.create_index(f"idx_{_TABLE}_expires_at", _TABLE, ["expires_at"], if_not_exists=True)
