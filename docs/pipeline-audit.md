...
| ~~Medium~~ **Fixed** | Some tests left SQLAlchemy/aiosqlite resources alive across separate `asyncio.run()` loops, producing connection-finalizer warnings at teardown. | Fixed in `tests/conftest.py`: an autouse fixture wraps `OrmDatabase.__init__` for the duration of each test and disposes every engine the test created, so the discipline holds for tests not yet written rather than being remembered file by file. The suite now ends with no unraisable-exception warnings. |
